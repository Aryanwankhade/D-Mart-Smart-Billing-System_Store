...
def apply_coupon():
    ...
    if total < min_order:
        shortfall = round(min_order - total, 2)
        return jsonify({
            'success': False,
            'message': f"Minimum order of Rs.{min_order:.0f} required for this coupon. "
                       f"Add Rs.{shortfall:.2f} more to your cart."
        })
    ...
    return jsonify({'success': True, 'discount': disc, 'desc': coupon['desc'],
                    'message': f"Coupon applied! You save Rs.{disc:.2f}"})
